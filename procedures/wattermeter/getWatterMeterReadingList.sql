  ...
  declare _result JSON; 
   ...
   -- Eliminar tablas temporales
    ...
    CREATE TEMPORARY TABLE fechas_unicas AS (
        SELECT idContador, DATE(fecha) AS fecha_dia, 
            CASE
    ...
    CREATE TEMPORARY TABLE lecturas_combinadas AS (
        SELECT f.idcontador, f.fecha_dia, f.periodo, 
        	c.valorCloro AS valor_cl, 
        	p.valorPh AS valor_ph, 
        	v.valorMCubicos AS valor_volumen
        FROM fechas_unicas f
        LEFT JOIN (SELECT idContador, DATE(fecha) AS fecha_dia, 
            CASE
                WHEN TIME(fecha) < '14:00:00' THEN 'M'
                ELSE 'T'
            END AS periodo, valorCloro FROM lecturascl) c 
            ON f.idContador = c.idContador AND f.fecha_dia = c.fecha_dia AND f.periodo = c.periodo
        LEFT JOIN (SELECT idContador, DATE(fecha) AS fecha_dia, 
            CASE
                WHEN TIME(fecha) < '14:00:00' THEN 'M'
                ELSE 'T'
            END AS periodo, valorPh FROM lecturasph) p 
            ON f.idContador = p.idContador AND f.fecha_dia = p.fecha_dia AND f.periodo = p.periodo
        LEFT JOIN (SELECT idContador, DATE(fecha) AS fecha_dia, 
            CASE
                WHEN TIME(fecha) < '14:00:00' THEN 'M'
                ELSE 'T'
            END AS periodo, valorMCubicos FROM lecturasmetroscubicos) v 
            ON f.idContador = v.idContador AND f.fecha_dia = v.fecha_dia AND f.periodo = v.periodo
    );
